...
- **Dataset V (Phase 2 step 5 output)** at `Flare/training/data/` (gitignored, reproducible):
  ...
  - Generated by deterministic `Flare/training/capture/generate_sessions.py` → `FlareLab/sessions/session_0{1..5}_*.bin` (no human `--record` involved — see Session 6 notes).
...
### Session 6 — 2026-05-19 (post-AFK)

Session ended abruptly when the PC powered off mid-AFK (suspected mains glitch). Verification on resume confirmed Phase 2 step 5 had actually finished writing all 5 dataset shards before the shutdown; this session just commits the artifacts and updates the doc.

**Done**
- **Phase 2 step 5 — synthetic-replay dataset capture pipeline** (`Flare/training/capture/`):
  - `generate_sessions.py` — deterministic synthesizer for 5 input-log archetypes (idle / slow / chase / scrum / empty) matching the Phase-2-step-2 wire format. 7200 frames @ 60 FPS each, Gaussian-jittered around parametric trajectories (Down at frame 0, Move every frame, Up at last). Fixed per-archetype seeds (`0xF1A8E_001..005`).
  - `run_pipeline.py` — for each `.bin`: replays through `flarelab.exe --replay --capture`, parses the first Sample header for sanity metadata, zstd-compresses with python `zstandard` level 1, writes `meta.json` with capture date + repo SHAs + GT settings + first-sample summary.
- **Pipeline executed end-to-end before the power loss.** 5 × `Flare/training/data/session_0{1..5}_*/samples.zst` (~4.8 MB each) + `meta.json` exist. Per session: 1830 samples × 263336 B = 481.9 MB raw → ~1% zstd. captured_at timestamps 18:17:29 → 18:20:31 (~45 s replay each — FlareLab `--replay` runs faster than vsync). All metas reference frozen SHAs `flarelab_sha=608f341 / ithappygame_sha=20256b9`.
- **Phase 2 closed.**

**Methodology divergence from the prior plan (worth flagging for the paper)**
...
With Dataset V frozen, Phase 3 is model design + training. Concrete first task:

1. **Write `Flare/training/MODEL_SPEC.md`** (1 page, design-doc style, *no code yet*):
   - **Input encoding.** 256² depth (f16, captured) + per-pixel light feature stack (8 lights × {posR, colAtt} = 64 channels broadcast? or a smaller compressed encoding?). Camera VP also available for world-pos reconstruction in-network. Open question: feed raw depth + light list, or pre-bake world-pos + light-direction-per-pixel?
   - **Output.** 256² shadow factor (f16, matching DATA_SPEC field). Single channel.
   - **Architecture candidates.** (a) Lightweight U-Net (4 down / 4 up, skip connections); (b) flat conv stack with dilated convs (no down/up, depth-only field of view); (c) per-light then aggregate (one tiny CNN per light, sum). Pick one with NPU-friendly ops as primary, log alternatives.
   - **Loss.** L1 on shadow factor + maybe L1 on its spatial gradient for edge sharpness. No perceptual loss yet.
   - **Train / val split.** 4 sessions train + 1 session held out. Which session held out? Recommend `session_05_empty` (least busy → cleanest GT distribution shift signal).
   - **Hardware path.** PyTorch on RTX 4060 Ti, FP16 mixed precision. ONNX export gated by NNAPI op coverage check.
   - **Success criterion.** SSIM ≥ X vs PCF GT on held-out session (X TBD by Phase-5 perceptual threshold work; placeholder 0.95).

2. **Discuss MODEL_SPEC before any training-loop code.** Plan-before-implement applies harder here than for the data pipeline — bad architecture choices cost weeks.

**Out of scope until later phases:** NNAPI/TFLite (Phase 4), eval harness (Phase 5), user study (Phase 6), paper (Phase 7).
